...
- **RQ1 — Rain, frost and accident risk:** How do varying intensities of precipitation (light vs. heavy rain) and freezing temperatures, which make roads wet or icy, affect accident frequency, severity, and accident type (e.g. loss-of-control vs. collision), compared across dry, wet and icy road conditions?
...
| **Spatial join (accident → nearest station)** | Cleaned accident CSV + stations CSV | Accident CSV with `station_id` column (`data/joined/accidents_stations.csv`) | Custom Python CLI — `src/join_data.py` using `scipy.spatial.cKDTree` | Completely runnable locally — pure in-memory KD-tree operation over the previously prepared files. |
